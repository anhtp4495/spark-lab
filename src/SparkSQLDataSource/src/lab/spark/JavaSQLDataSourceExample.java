...
public class JavaSQLDataSourceExample {
	...
	private static void runGenericFileSourceOptionsExample(SparkSession spark) {
		...
		Dataset<Row> testCorruptDF = spark.read().parquet("resources/dir1/", "resources/dir1/dir2/");
		testCorruptDF.show();
		// +-------------+
		// | file|
		// +-------------+
		...
		Dataset<Row> recursiveLoadedDF = spark.read().format("parquet").option("recursiveFileLookup", "true")
				.load("resources/dir1");
		...
		Dataset<Row> testGlobFilterDF = spark.read().format("parquet").option("pathGlobFilter", "*.parquet") // json
																												// file
																												// should
																												// be
																												// filtered
		...
		Dataset<Row> beforeFilterDF = spark.read().format("parquet")
				...
				.option("timeZone", "CST").load("resources/dir1");
		beforeFilterDF.show();
		// +-------------+
		// | file|
		// +-------------+
	...
	private static void runBasicDataSourceExample(SparkSession spark) {
		...
		Dataset<Row> peopleDF = spark.read().format("json").load("resources/people.json");
		peopleDF.select("name", "age").write().format("parquet").save("namesAndAges.parquet");
		// $example off:manual_load_options$
		// $example on:manual_load_options_csv$
		Dataset<Row> peopleDFCsv = spark.read().format("csv").option("sep", ";").option("inferSchema", "true")
				.option("header", "true").load("resources/people.csv");
		// $example off:manual_load_options_csv$
		// $example on:manual_save_options_orc$
		usersDF.write().format("orc").option("orc.bloom.filter.columns", "favorite_color")
				.option("orc.dictionary.key.threshold", "1.0").option("orc.column.encoding.direct", "name")
				.save("users_with_options.orc");
		// $example off:manual_save_options_orc$
		// $example on:direct_sql$
		Dataset<Row> sqlDF = spark.sql("SELECT * FROM parquet.`resources/users.parquet`");
	...
	private static void runJsonDatasetExample(SparkSession spark) {
		...
		Dataset<Row> people = spark.read().json("resources/people.json");
	...
	private static void runJdbcDatasetExample(SparkSession spark) {
		...
		Dataset<Row> jdbcDF = spark.read().format("jdbc").option("url", "jdbc:postgresql:dbserver")
				.option("dbtable", "schema.tablename").option("user", "username").option("password", "password").load();
	...
	private static void cleanup() {
		try {
			FileUtils.deleteDirectory(new File("namesAndAges.parquet"));
		} catch (IOException e) {
			e.printStackTrace();
		}

		try {
			FileUtils.deleteDirectory(new File("namesAndFavColors.parquet"));
		} catch (IOException e) {
			e.printStackTrace();
		}

		try {
			FileUtils.deleteDirectory(new File("namesPartByColor.parquet"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		try {
		...
		try {
			FileUtils.deleteDirectory(new File("users_with_options.orc"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
