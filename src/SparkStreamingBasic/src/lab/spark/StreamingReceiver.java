 ...
 */
public class StreamingReceiver extends Receiver<String> {
	private static final Pattern SPACE = Pattern.compile(" ");

	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			System.err.println("Usage: StreamingReceiver <hostname> <port> <second_duration>");
			System.exit(1);
		}
		...
		duration = (duration > 0) ? duration : 1L; // 1 second
		// StreamingExamples.setStreamingLogLevels();

		// Create the context with a 1 second batch size
		SparkConf sparkConf = new SparkConf().setAppName("JavaStreamingReceiver");
		JavaStreamingContext ssc = new JavaStreamingContext(sparkConf, Durations.seconds(duration));

		// Create an input stream with the custom receiver on target ip:port and count
		// the words in input stream of \n delimited text (e.g. generated by 'nc')
		StreamingReceiver receiverStream = new StreamingReceiver(args[0], Integer.parseInt(args[1]));

		System.out.println("Processing stream ...");
		JavaReceiverInputDStream<String> lines = ssc.receiverStream(receiverStream);
	...
	public StreamingReceiver(String host_, int port_) {
